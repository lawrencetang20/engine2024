...
tracks over 50 hands - number of losses by under 20:
    - if win more than half (25), keep betting strat
    - if losing more than half and more than 85% by less than 20, increase range
    - if losing more than half and less than 85% by less than 20, decrease range
'''
...
class Player(Bot):
    ...
    def __init__(self):
        ...
        self.hands_lost_less_than_20 = 0
        self.rounds_lost = 0
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        if my_delta < 0 and my_delta >= -20:
            self.hands_lost_less_than_20 += 1 
        ...
        if self.total_rounds % 50 == 0:
            self.range_adj = self.get_range_adjustment(self.rounds_lost,self.hands_lost_less_than_20)
            print(self.get_range_adjustment(self.rounds_lost,self.hands_lost_less_than_20))
        if street>=3:
    ...
    def get_range_adjustment(self,rounds_lost,rounds_lost_under_20):
        if rounds_lost < 25:
            return 0
        else:
            return rounds_lost_under_20-25
    ...
    def get_preflop_action(self,cards,round_state,active):
        ...
        if big_blind == False and self.times_bet_preflop == 0:
            ...
            elif self.preflop_dict[new_cards] in range(8,int(self.big_blind_call+1-((opp_pip-2)/198)**(1/3)*(self.big_blind_call+1-8))+self.range_adj) and opp_pip <= 200:
            ...
            if self.preflop_dict[new_cards] in range(1,6):
                ...
                if RaiseAction in legal_actions:
                    ...
                    print("this shouldn't ever happen")
            elif self.preflop_dict[new_cards] in range(6, int(67-((opp_pip-2)/398)**(1/3)*61)+self.range_adj):
                if CallAction in legal_actions:
                    return CallAction()
                else:
                    return CheckAction()
    ...
    def get_board_dryness(self,round_state):
        board_cards = round_state.deck[:3]  # the board cards
        card1 = board_cards[0]
        card2 = board_cards[1]
        card3 = board_cards[2]
        o = 'AKQJT98765432'
        ...
        if suit1 != suit2 != suit3:
            ...
            flush_draw = 1 
        if max(o.index(rank1),o.index(rank2),o.index(rank3))-min(o.index(rank1),o.index(rank2),o.index(rank3)) > 5:
            straight_draw = 0
        elif max(o.index(rank1),o.index(rank2),o.index(rank3))-min(o.index(rank1),o.index(rank2),o.index(rank3)) in (4,5):
            straight_draw = 1 
        else:
            straight_draw = 2
        if len({suit1,suit2,suit3}) == 3:
            paired = 0
        elif len({suit1,suit2,suit3}) == 2:
            paired = 1
        else:
            paired = 2
        
        return flush_draw+straight_draw+paired
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # if street == 3:
        #     hand_strength -= self.get_board_dryness(round_state)*0.05
        ...
        if BidAction in legal_actions:
            ...
            decision, conf = self.decide_action_postflop(opp_pip, my_pip, hand_strength, pot, legal_actions, street, my_bid, opp_bid)
